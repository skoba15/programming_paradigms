                ...
                AccountAmount amount)
{
   ...
   pthread_mutex_lock(&(bank->transferlocker));
  ...
  if (amountAbs < reportingAmount) {
    pthread_mutex_unlock(&(bank->transferlocker));
...
Report_DoReport(Bank *bank, int workerNum)
{
  ...
  pthread_mutex_lock(&(bank->countchecker));
  bank->endedworkingday++;
  if(bank->endedworkingday==bank->numofworkers){
     Report *rpt = bank->report;
  assert(rpt); Y;
  ...
  if (rpt->numReports >= MAX_NUM_REPORTS) {
      ...
      for(int i=0; i<bank->numofworkers-1; ++i){
	sem_post(&(bank->semm));
      }
	pthread_mutex_unlock(&(bank->countchecker));
      return -1;
  }
