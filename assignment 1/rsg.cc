...
void recursion(string nonterminal, map<string, Definition> & grammar){
  Definition d=grammar[nonterminal];
  Production pp= d.getRandomProduction();
  for(Production::iterator it=pp.begin(); it!=pp.end(); ++it){
    string s=*(it);
    if(s[0]=='<'){
      recursion(s, grammar);
    }
...
int main(int argc, char *argv[])
{
  ...
  for(int i=0; i<3; ++i){
     ...
     Production products=d.getRandomProduction();
    cout<<"Version #"<<i+1<<endl;
    for(Production::iterator it=products.begin(); it!=products.end(); ++it){
      string s=*(it);
      if(s[0]=='<'){
	recursion(s, grammar);
      }
